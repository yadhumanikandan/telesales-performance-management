...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!allowedRoles.includes(roleData.role)) {
      return new Response(
        JSON.stringify({ error: "Insufficient permissions to check performance alerts" }),
        { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    console.log(`Performance alert check initiated by user ${user.id} with role ${roleData.role}`);
