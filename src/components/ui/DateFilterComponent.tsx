...
export const DateFilterComponent = ({
  ...
  return (
    <div className={cn("flex flex-col gap-6 p-4 bg-card border rounded-lg", className)}>
      ...
      {/* Step 2: Date Pickers (conditional) */}
      <div className="flex flex-col gap-3">
        <h3 className="text-sm font-semibold text-foreground">Step 2: Select Date(s)</h3>
        
        {/* Show instruction until user selects a mode */}
        {selectionMode === null && (
      ...
      {/* Step 3: Agent Filter */}
      <div className="flex flex-col gap-3">
        <h3 className="text-sm font-semibold text-foreground">Step 3: Select Agent (Optional)</h3>
        <select
          value={selectedAgent}
          onChange={(e) => setSelectedAgent(e.target.value)}
          className="h-10 px-3 rounded-md border border-input bg-background text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 w-full sm:w-[250px]"
        >
      ...
      <div className="flex flex-wrap gap-3 pt-2 border-t">
        ...
        <Button
          ...
          className="h-10 min-w-[140px] ml-auto"
        >
          {isExporting ? (
            <>
              <Loader2 className="h-4 w-4 mr-2 animate-spin" />
